...
from . import (envIAMData, AWS_POLICIES, createDir, 
               ...
               filteringListEntitiesForPolicy, checkingLEFPPermission, scanningListEntitiesForPolicy, populateAMPforPoliciesAllCase,
...
def multiAccountThreading(output_folder, account_id, credential_list):
    ...
    # Re-scan by Reverse Approach - ListEntitiesForPolicy - ListPolicies
    envPolicies, reScanNamePolicies = filteringListEntitiesForPolicy(envData.users, envData.groups, envData.roles)
    if reScanNamePolicies.get("Users") or reScanNamePolicies.get("Groups") or reScanNamePolicies.get("Roles"):
        condition_LEFP = False
        for session in session_list:
            if checkingLEFPPermission(session.client('iam')):
                condition_LEFP = True
        if condition_LEFP and not stop_event.is_set():
            ...
            logging.info("Attempting to initialize IAM [ListPolicies / ListEntitiesForPolicy] Inverse Enumeration Model to complement...")
            if not stop_event.is_set():
                with ThreadPoolExecutor(max_workers=resources.threads_config.MAX_THREADS) as sub_executor:
                    futures = [
                        sub_executor.submit(scanningListEntitiesForPolicy,
                            session.client("iam"),
                            reScanNamePolicies,
                            AWS_POLICIES,
                            envData
