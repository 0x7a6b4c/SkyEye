...
def process_credential_set_cross(session, sts_caller_identity, sts_caller_identity_list, output_folder, stop_event,
                                envData,
                                mode="default"):
    iam_client = session.client("iam")
    ...
    try:
        output = getAccountAuthorizationDetailsEnumCross(iam_client, sts_caller_identity, target_user_arns, stop_event,
                                                envData,
                                                mode)
    except Exception as e:
        logging.error(f"Error processing credentials set {sts_caller_identity['AccessKey']}: {e}", exc_info=True)
